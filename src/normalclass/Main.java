...
public class Main {
	...
	public static void main(String[] args) {
		...
		first = new FirstPage();
		/*user = new User();
		user.setMyCard(new Card(2,1));
		user.setMyCard(new Card(2,2));
		user.setMyCard(new Card(3,4));
		user.setMyCard(new Card(9,3));
		//user.setMyCard(new Card(14,1));
		//user.setMyCard(new Card(5,4));
		//user.setMyCard(new Card(9,3));
		analyzerB.analysis(user.getMycard());
		user.setGrade(analyzerB.getTmpGrade(0), analyzerB.getTmpGrade(1));
		analyzerB.initA();
		
		float[] arr = analyzerPE.analysis(user.getMycard(), user, "ㅇ", 3);
		
		System.out.println(Arrays.toString(arr));*/
	}
	//올인할 때 내가 콜을 해도 다음턴 사람은 50만원을 걸어야하는데 판돈이 2만원으로 바껴버림 그럼 안대지
